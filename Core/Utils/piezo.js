...
const engine = (() => {
  ...
  function play() {
    if (
      !raf &&
      (!isDocumentHidden() || !piezo.suspendWhenDocumentHidden) &&
  ...
  function handleVisibilityChange() {
    if (!piezo.suspendWhenDocumentHidden) return
...
function piezo(params = {}) {
  ...
  function resetTime() {
    startTime = 0
    lastTime = adjustTime(instance.currentTime) * (1 / piezo.speed)
  }
  ...
  instance.tick = function (t) {
    ...
    setInstanceProgress((now + (lastTime - startTime)) * piezo.speed)
  }
...
piezo.suspendWhenDocumentHidden = true
piezo.running = activeInstances
piezo.remove = removeTargetsFromActiveInstances
piezo.get = getOriginalTargetValue
piezo.set = setTargetsValue
piezo.convertPx = convertPxToUnit
piezo.path = getPath
piezo.setDashoffset = setDashoffset
piezo.stagger = stagger
piezo.timeline = timeline
piezo.easing = parseEasings
piezo.penner = penner
piezo.random = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min
